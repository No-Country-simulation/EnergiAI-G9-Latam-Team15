...
Artefactos requeridos (en ml-service/models/, carpeta hermana de app/)
──────────
    modelo_eficiencia_energetica.pkl   → Pipeline sklearn (preprocesador + RF)
    label_encoder.pkl                  → LabelEncoder para decodificar clases

    Ubicación sobrescribible con la variable de entorno MODELS_DIR.
══════════════════════════════════════════════════════════════════
"""
...
# ── Rutas de artefactos ──────────────────────────────────────
# Este archivo vive en ml-service/app/. Los artefactos (.pkl) viven
# en ml-service/models/ (carpeta HERMANA, no la misma). Se resuelve
# así por defecto; sobrescribible con la variable de entorno
# MODELS_DIR (útil en despliegue: Docker, otra ruta de montaje, etc.)
#
#   ml-service/
#   ├── app/
#   │   └── inference.py              ←  este archivo
#   └── models/
...
MODEL_PATH = os.path.join(MODELS_DIR, "modelo_eficiencia_energetica.pkl")
...
def _load_artifacts() -> None:
    ...
    if _pipeline is None:
        if not os.path.isfile(MODEL_PATH):
            raise FileNotFoundError(
                ...
                f"Se esperaba en 'ml-service/models/' (carpeta hermana de 'app/').\n"
                f"Para usar otra ubicación, defina la variable de entorno MODELS_DIR."
            )
    ...
    if _le is None:
        if not os.path.isfile(LE_PATH):
            raise FileNotFoundError(
                f"No se encontró el LabelEncoder en: {LE_PATH}\n"
                f"Para usar otra ubicación, defina la variable de entorno MODELS_DIR."
            )
